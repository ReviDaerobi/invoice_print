    ...
    <header class="bg-white shadow-sm z-10">
        <div class="px-4 sm:px-6 lg:px-8 py-4 flex items-center justify-between">
            ...
            <div class="flex-1 md:ml-0 md:mr-auto">
                <h2 class="text-xl font-semibold text-gray-800">Edit Delivery Order</h2>
            </div>
        </div>
    </header>
        ...
        <div class="max-w-4xl mx-auto">
            <div class="bg-white shadow-md rounded-lg overflow-hidden">
                <div class="p-4 sm:p-6 border-b">
                    ...
                    <p class="mt-1 text-sm text-gray-500">Edit informasi delivery order di bawah ini.</p>
                </div>
                
                <form action="{{ route('delivery-order.update', $deliveryOrder->id) }}" method="POST" class="p-4 sm:p-6 space-y-6">
                    @csrf
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <input type="text" name="no_do" id="no_do" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500" value="{{ old('no_do', $deliveryOrder->no_do) }}" required>
                            @error('no_do')
                        ...
                        <div>
                            ...
                            <input type="date" name="tanggal_do" id="tanggal_do" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500" value="{{ old('tanggal_do', $deliveryOrder->tanggal_do) }}" required>
                            @error('tanggal_do')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <select name="purchase_order_id" id="purchase_order_id" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                                ...
                                @foreach($purchaseOrders as $po)
                                    <option value="{{ $po->id }}" {{ old('purchase_order_id', $deliveryOrder->purchase_order_id) == $po->id ? 'selected' : '' }}>
                        ...
                        <div>
                            ...
                            <input type="text" name="no_po_customer" id="no_po_customer" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500" value="{{ old('no_po_customer', $deliveryOrder->no_po_customer) }}">
                            ...
                            <div id="items-container">
                                ...
                                <div class="item-row grid grid-cols-1 md:grid-cols-4 gap-4 mb-4">
                                    <div>
                                        ...
                                        <input type="text" name="items[{{ $index }}][nama_barang]" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500" value="{{ old('items.'.$index.'.nama_barang', $detail->nama_barang) }}" required>
                                        <input type="hidden" name="items[{{ $index }}][id]" value="{{ $detail->id }}">
                                    </div>
                                    <div>
                                        <label class="block text-sm font-medium text-gray-700">Jumlah</label>
                                        <input type="number" name="items[{{ $index }}][jumlah]" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500" min="1" value="{{ old('items.'.$index.'.jumlah', $detail->jumlah) }}" required>
                                    </div>
                                    <div>
                                        <label class="block text-sm font-medium text-gray-700">Satuan</label>
                                        <input type="text" name="items[{{ $index }}][satuan]" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500" value="{{ old('items.'.$index.'.satuan', $detail->satuan) }}" required>
                                    </div>
                                ...
                                @empty
                                <!-- If no existing items, show one empty item row -->
                                <div class="item-row grid grid-cols-1 md:grid-cols-4 gap-4 mb-4">
                                    <div>
                                        <label class="block text-sm font-medium text-gray-700">Nama Barang</label>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        let itemIndex = {{ count($deliveryOrder->details ?? [0]) }}; // Start from existing items count
        ...
        // Initialize remove button for all items
        attachRemoveItemEventListener();
    });
</script>
@endsection